# Filename: W1205_GenAI_LC_MemoryCache_04.py
...
def save_to_file(student_id, question, answer):
    ...
    filename = os.path.join(output_dir, f"W12_Assignment5_{student_id}_{datetime.datetime.now().strftime('%Y%m%d')}.txt")
    try:
        with open(filename, "w", encoding="utf-8") as file:
            file.write(f"W12 Assignment 5 | Student ID: {student_id} | {datetime.datetime.now().strftime('%Y%m%d')}\n")
            file.write("-----------------------------\n")
